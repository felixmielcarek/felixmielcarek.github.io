...
class GitHubCarouselLoader {
  ...
  isCacheValid() {
    ...
    if (version !== null && version !== CACHE_VERSION) {
      console.log(
        `🔄 Cache version mismatch (${version} !== ${CACHE_VERSION}), invalidating...`,
      );
      this.clearCache();
      return false;
    }
  ...
  getCachedRepos() {
    ...
    try {
      ...
      if (cached) {
        ...
        console.log(
          `✅ Loaded ${repos.length} portfolio repositories from cache (${sizeKB} KB)`,
        );
        return repos;
      }
  ...
  saveToCache(repos) {
    ...
    try {
      ...
      console.log(
        `✅ Cache saved successfully (${sizeKB} KB, version ${CACHE_VERSION})`,
      );
    } catch (error) {
  ...
  async fetchRepositories() {
    ...
    try {
      ...
      console.log(
        `🎯 Filtered to ${this.repos.length} portfolio repos (saved ${allRepos.length - this.repos.length} from cache)`,
      );
  ...
  filterByTopic(topic) {
    console.log(
      `🔍 Filtering ${this.repos.length} portfolio repos by topic: "${topic}"`,
    );
    ...
    const filtered = this.repos.filter((repo) => {
      ...
      if (hasTopics) {
        console.log(
          `  ✓ Found: ${repo.name} (topics: ${repo.topics.join(", ")})`,
        );
      }
      return hasTopics;
    });
  ...
  createProjectCard(repo, type) {
    ...
    if (type === "university") {
      ...
      extraContent = `
                <div class="d-flex gap-3 mb-2">
                    <small class="text-muted">Stars: ${repo.stargazers_count}</small>
                    <small class="text-muted">Watchers: ${repo.watchers_count}</small>
                </div>
  ...
  populateCarousel(carouselId, repos, type) {
    ...
    // Create indicators
    if (carouselIndicators && slidesData.length > 1) {
    ...
    // Hide carousel controls and indicators if only one slide
  ...
  async init() {
    ...
    console.log(
      `📦 Production: ${prodRepos.length} | Dev: ${devRepos.length} | Backlog: ${backlogRepos.length} | University: ${uniRepos.length}`,
    );
    ...
    if (cachedData) {
      ...
      const cacheAge = timestamp
        ? ((Date.now() - parseInt(timestamp)) / (60 * 60 * 1000)).toFixed(1)
        : "unknown";
